// __tests__/api/manager-approvals/RejectTimesheet-route.test.ts

jest.mock('@/server/sql/sqlHandler.server', () => ({
  query: jest.fn(),
}));

import { POST } from '@/app/api/manager-approvals/RejectTimesheet/route';
import { query } from '@/server/sql/sqlHandler.server';

describe('POST /api/manager-approvals/RejectTimesheet', () => {
  ...
  // Helper to stub Request.url properly
  const makeRequest = (qs: string): Request =>
    ({ url: `http://localhost/api/manager-approvals/RejectTimesheet${qs}` } as unknown as Request);
  ...
  it('returns 400 if timesheetentryid is not a number', async () => {
    const req = makeRequest('?timesheetentryid=abc');
    const res = await POST(req);
  ...
  it('returns 200 and the query result on success', async () => {
    ...
    const req = makeRequest('?timesheetentryid=123');
    const res = await POST(req);
  ...
  it('returns 500 if the database call throws', async () => {
    ...
    const req = makeRequest('?timesheetentryid=5');
    const res = await POST(req);
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      'Error setting timesheet rejection:',
      dbError
    );
